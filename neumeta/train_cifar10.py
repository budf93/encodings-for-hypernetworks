...
def initialize_wandb(config):
    ...
    # Set your API key here to avoid interactive login
    os.environ["WANDB_API_KEY"] = "INSERT_API_KEY"
    wandb.login()
...
def train_one_epoch(model, train_loader, optimizer, criterion, dim_dict, gt_model_dict, epoch_idx, ema=None, args=None):
    ...
    # print(f'gt_model_dict: {gt_model_dict}')
    ...
    for batch_idx, (x, target) in enumerate(train_loader):
        ...
        # change to other probability distributions?
        # Add noise to the coordinates if specified
        if args.training.coordinate_noise > 0.0:
            noise_type = args.training.noise_type.lower()
            
            if noise_type == 'uniform':
                # Uniform noise in [-0.5, 0.5], scaled by coordinate_noise
                noise = (torch.rand_like(coords_tensor) - 0.5) * args.training.coordinate_noise
                coords_tensor = coords_tensor + noise

            elif noise_type == 'gaussian':
                # Gaussian noise with mean 0, std adjusted by coordinate_noise
                noise = torch.randn_like(coords_tensor) * args.training.coordinate_noise
                coords_tensor = coords_tensor + noise

            elif noise_type == 'laplacian':
                # Laplace noise with location 0 and scale (b = coordinate_noise / sqrt(2))
                scale = args.training.coordinate_noise / (2 ** 0.5)
                laplace = torch.distributions.Laplace(loc=0.0, scale=scale)
                noise = laplace.sample(coords_tensor.shape).to(coords_tensor.device)
                coords_tensor = coords_tensor + noise

            elif noise_type == 'adversarial':
                # Placeholder for adversarial coordinate noise (gradient-based or optimization-driven)
                # For now, this is a stub — requires implementation based on a loss gradient.
                # Example:
                # coords_tensor.requires_grad_(True)
                # output = model_cls(coords_tensor)  # hypothetical forward pass
                # loss = some_loss_fn(output, target)
                # loss.backward()
                # coords_tensor = coords_tensor + args.training.coordinate_noise * coords_tensor.grad.sign()
                raise NotImplementedError("Adversarial noise not yet implemented")
